...
const EMPTY = { text: '', name: '', detail: '', stars: 5, display_order: '', active: true }
...
export default function TestimonialsEditPage() {
  ...
  async function handleSave(e) {
    ...
    if (isNew) navigate('/testimonials', { replace: true })
    else setMessage({ type: 'success', text: 'Saved successfully.' })
  }

  async function handleDelete() {
    if (!window.confirm(`Delete testimonial from "${form.name}"?`)) return
    setDeleting(true)
  ...
  return (
    <Layout>
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate('/testimonials')} className="text-blue-600 hover:underline text-sm">← Back</button>
        <h2 className="text-xl font-bold text-gray-800">{isNew ? 'New Testimonial' : 'Edit Testimonial'}</h2>
      </div>

      <form onSubmit={handleSave} className="space-y-6">
        <div className="bg-white rounded-xl shadow p-6 space-y-4">
          <Field label="Quote text *">
            <textarea rows={4} required value={form.text} onChange={e => set('text', e.target.value)} className={inputCls} />
          </Field>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Member detail (e.g. Personal Training Client)">
              <input type="text" value={form.detail} onChange={e => set('detail', e.target.value)} className={inputCls} />
            </Field>
            <Field label="Stars (1–5)">
              <select value={form.stars} onChange={e => set('stars', e.target.value)} className={inputCls}>
