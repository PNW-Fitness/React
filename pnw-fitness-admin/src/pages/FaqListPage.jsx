            ...
            <tbody className="divide-y divide-gray-100">
              {items.map(f => (
                <tr key={f.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-400">{f.display_order}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => navigate(`/faq/${f.id}`)} className="text-blue-600 hover:underline text-sm">Edit</button>
                  </td>
                </tr>
              ))}
              {items.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-6 text-center text-gray-400">No questions yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
