...
function getTodayWindow(holiday) {
  if (holiday?.status === 'closed') return null
  if (!holiday || holiday.status === 'regular') {
    const weekend = [0, 6].includes(new Date().getDay())
    return { openAt: weekend ? 480 : 360, closeAt: weekend ? 1200 : 1260 }
  }
  return { openAt: holiday.open_at_minutes, closeAt: holiday.close_at_minutes }
}

function getStatus(holiday) {
  const now = new Date()
  const mins = now.getHours() * 60 + now.getMinutes()
  const window = getTodayWindow(holiday)
...
function getTodayLabel(holiday) {
  if (holiday) return holiday.label
  return [0, 6].includes(new Date().getDay()) ? 'Saturday – Sunday' : 'Monday – Friday'
}

function getTodayHours(holiday) {
  if (holiday?.status === 'closed') return 'Closed Today'
  const window = getTodayWindow(holiday)
    ...
    <section id="home" style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0d0d0d 0%,#141414 50%,#222222 100%)', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'flex-end', padding: '0 56px 80px' }}>
      ...
      <div style={{ position: 'fixed', top: '90px', right: '2.5%', zIndex: 500, background: 'rgba(20,20,20,0.92)', backdropFilter: 'blur(16px)', border: '1px solid rgba(37,99,235,0.25)', borderRadius: '16px', padding: '18px 22px', minWidth: '230px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
          ...
          <div>
            <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', marginBottom: '2px' }}>{getTodayLabel(holiday)}</div>
            <div style={{ fontSize: '12px', color: '#fff', fontWeight: 600 }}>{getTodayHours(holiday)}</div>
          </div>
        </div>
