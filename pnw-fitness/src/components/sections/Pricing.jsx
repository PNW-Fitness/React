...
export default function Pricing({ onJoinClick }) {
  ...
  const plan1 = plans[0]
  const plan2 = plans[1]
  const yearlyEquiv  = plan2?.yearly_price ? (plan2.yearly_price / 12).toFixed(2) : null
  const savings      = plan2?.yearly_price ? ((plan2.monthly_price * 12) - plan2.yearly_price).toFixed(2) : null
  const savingsPct   = plan2?.yearly_price ? Math.round((1 - plan2.yearly_price / (plan2.monthly_price * 12)) * 100) : null
    ...
    >
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '48px' }}>
          ...
          {/* Toggle — affects yearly-capable plan only */}
          {plan2?.yearly_price && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', alignItems: 'start' }}>
            ...
            {plan1 && (
              <div style={{
                ...
                >
                  Join {plan1.name} — ${plan1.monthly_price}/mo
                </button>
              </div>
            )}

            {/* Plan 2 — yearly toggle applies here */}
            {plan2 && (
              <div style={{
                background: '#141414',
                ...
                <div style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#2563EB', marginBottom: '8px' }}>
                  {yearly && plan2.yearly_price ? 'Paid in Full' : 'Month to Month'}
                </div>
                <div style={{ fontFamily: 'Barlow Condensed, sans-serif', fontSize: '18px', fontWeight: 700, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '24px' }}>
                  {yearly && plan2.yearly_price ? 'Annual' : 'Membership'}
                </div>

                {yearly && plan2.yearly_price && (
                  <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', textDecoration: 'line-through', marginBottom: '4px' }}>
                    ${plan2.monthly_price}/mo
                  </div>
                )}
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', marginBottom: '6px' }}>
                  <span style={{ fontFamily: 'Barlow Condensed, sans-serif', fontSize: '60px', fontWeight: 900, color: '#fff', lineHeight: 1 }}>
                    {yearly && plan2.yearly_price ? `$${yearlyEquiv}` : `$${plan2.monthly_price}`}
                  </span>
                  <span style={{ fontSize: '14px', color: 'rgba(255,255,255,0.4)' }}>/mo</span>
                </div>
                ...
                {yearly && plan2.yearly_price ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '32px' }}>
                    <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>
                      Billed as <strong style={{ color: '#fff' }}>${plan2.yearly_price}</strong>/yr
                    </span>
                    <span style={{
                      fontSize: '10px',
                      fontWeight: 800,
                ...
                >
                  {yearly && plan2.yearly_price
                    ? `Join Now — $${plan2.yearly_price}/yr`
                    : `Join Now — $${plan2.monthly_price}/mo`}
                </button>
              </div>
            )}
          </div>
